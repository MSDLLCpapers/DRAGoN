...
workflow Preprocessing {
    ...
    fastqc_in = fastqc_in.mix(
        reads_in.map { meta, fastq, _barcodes ->
            [meta + [filetype: 'fastq', stage: 'untrimmed_fastqc'], fastq]
        }
    )

    // Reinstate support for --Demux.dmuxn
    // Limit to 500M reads per job so as to keep file sizes low for transfer
    ...
    fastqc_in = fastqc_in.mix(
        Decontaminate.out.decontaminated.map { meta, bam, _barcodes ->
    ...
    // Add key "plate_map"
    MergeDemuxJson(
        Demultiplex.out.log.map { meta, json, barcodes ->
            [meta.id, meta, json, barcodes]
        }.groupTuple().map { _id, meta, json, barcodes ->
            def x = [meta, json]
                .transpose()
                .sort { it -> it[0].id2 }
                .transpose()
    ...
    SAMTOOLS_SPLIT.out.bam
        ...
        .groupTuple()
        .combine(
            MergeDemuxJson.out.merged.map { meta, json, stdout ->
                [meta.id, json, stdout]
            },
            by: 0
        )
        ...
        .flatMap { _id, meta, bam, barcodes, json, stdout ->
            ...
            bam
                .flatten()
                .collect { fl ->
                    return (fl.baseName =~ /^(\S+)_unmap_(\w{4})$/).collect { m ->
                        def bcidx = m[2]
                        def bcname
                        if (bcidx != 'noBC') {
                            bcname = barcodes[0]
