...
process StageStarIndexToScratch {
    executor 'local'
    stageInMode 'copy'
    input:
        path staridx
    output:
        path staridx, emit: idx
    script:
    """
    ls
    """
...
workflow GatherGenomeFiles {
    ...
    main:
        ...
        if (file("$staridx/genomeParameters.txt").exists()) {
            ...
            staridx = Channel.value(file(staridx, type: 'dir').listFiles() as List)

            // To avoid network IO hangups, copy the STAR index into the local workdir
            if (file(workDir).scheme != 's3' && !val_no_prestage_index) {
                StageStarIndexToScratch(staridx)
                staridx = StageStarIndexToScratch.out.idx
            }
        } else {
